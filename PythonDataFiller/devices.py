...
def createHistory(userID):
    lightHistoryGenerator = range(random.randrange(1,30))
    fridgeHistoryGenerator = range(random.randrange(1,30))
    vacuumHistoryGenerator = range(random.randrange(1,30))
    for i in range(lightHistoryGenerator):
        colIoT_Device_History.insert_one({userID: {"1": createTheSmartLight()["Device_Status"]}})
    
    for i in range(fridgeHistoryGenerator):
        colIoT_Device_History.insert_one({userID: {"2": createTheFridge()["Device_Status"]}})
        
    for i in range(vacuumHistoryGenerator):
        colIoT_Device_History.insert_one({userID: {"3": createTheVauum()["Device_Status"]}})

createHistory("626b4821b343c835ea0ed734")